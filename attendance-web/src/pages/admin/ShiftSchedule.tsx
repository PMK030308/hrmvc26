...
export default function AdminShiftSchedule() {
  ...
  return (
    <div>
                      ...
                      return (
                        <td key={d} className="min-w-[72px] px-2 py-2 text-center">
                          <button onClick={() => { setCell({ empId: e.id, date: d }); setPickShift(sc?.shiftId ?? '') }}
                            className="mx-auto grid h-9 w-9 place-items-center rounded-lg text-xs font-bold text-white transition hover:scale-110"
      ...
      <Modal open={bulkOpen} onClose={closeBulk} size="md" title="Phân ca hàng loạt"
        footer={<>
